...
On the other side, instances of the Network Configuration custom resource are expected to be short, because they only require a list of all external networks connected to compute nodes of an OpenStack cluster.
...
This means that the containers from the OpenStack add-on operator and its child operators themselves run on the first project, and the containers which run componentes of Nova, Neutron, RabbitMQ, and other OpenStack services run on the second project.
