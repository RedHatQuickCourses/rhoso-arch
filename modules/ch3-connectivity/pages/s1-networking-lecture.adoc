...
There is no easy way of finding the IP ranges of the Kuberntes primary network. You could just check the IP addresses of individual cluster nodes, from the Node Kubernetes resource type, and guess which is their subnet range, or ask an OpenShift Administator.
