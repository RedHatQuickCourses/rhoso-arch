...
The following figure illustrates a generic data center environment, just to provide context and invite you to consider how an OpenStack cluster might fit in such an environment.
...
The reference design of OpenStack clusters includes isolated networks for OpenStack itself and its supporting infrastructure, such as external storage backends, and possibly multiple isolated networks dedicated to application traffic.

image::s1-networking-lecture-fig-2.svg[alt="Reference OpenStack network isolation scenario"]

All of those isolated networks must be attached to OpenStack cluster nodes, including the OpenShift cluster nodes which run an OpenStack control plane, and different components in the control and data planes of the OpenStack cluster require connectivity with different isolated networks. In fact, segregating traffic from different components, and from OpenStack itself from applications, is a major factor driving an isolated network design for OpenStack.
...
Connects OpenStack compute nodes to storage back-ends, to store application data managed by  OpenStack services, such as Cinder and Glance, on behalf of applications.
...
Option (3) above is the most popular with cloud-native applications, because it mixes well with API gateways and other cloud application design patterns. It also eases network management, because a single reverse proxy instance, thus a single external IP address, can be shared with many application instances.
...
A Kubernetes Service finds its Pods by means of a pod selector that matchs labels on Pods. Usually, a Service uses the same labels that a workload controller, such as a Deployment, uses to match those pods. But it doesn't have to use exactly the same labels, which enable sophisticated processes such as A/B application testing and canary deployments.

There are many types of Kubernetes Services, and the main ones are:

ClusterIP::
Are internal load balancers which front Pods for traffic originating from other Pods in the same cluster.

LoadBalancer::

Are external load balancers which front Pods for traffic originating outside of their clusters?
...
Kubernetes external load balancers, that is, Services of type `LoadBalancer`, have both an internal IP, inside the virtual internal network, and an external IP, or Virtual IP (VIP), which connects the load balancer to the outside of a cluster. 

=== Kubernetes Ingress and OpenShift Routes

OpenShift Route custom resources and Kubernetes Ingress resources serve similar purposes: reverse HTTP proxying, which enables traffic from outside a cluster to reach Pods inside a Kubernetes cluster. Routes and Ingress differ in syntax and minor features, and OpenShift Routes predate Kubernetes Ingress, much makes it more common among OpenShift users.
...
Red Hat OpenShift fills those gaps with the Ingress cluster operator, which provides an Ingress controller, and the MetalB add-on operator, which supports external load balancer services.
...
* MetalLB, is an OpenShift add-on operator which enables Kubernetes Services to work as external load balancers on physical servers.
...
Traffic to other Pods and to Kubernetes Services still flows through the primary network interface from Kubernetes, but secondary networks enable Pods to both starts and listen to network connections on those secondary networks, over any network protocol.

=== Multus Secondary Networks and Pods

Multus is a Container Network Interface plugin (CNI) that enables attaching multiple virtual network interfaces to Pods. Multus enables many interesting features, by the use of a plug-in architecture and a JSON configuration syntax, which are beyond the scope of this course.

Network Attachment Definition Custom resources represent secondary networks. Once there is a Network Attachment Definition in a project, Pods use annotations to declare the secondary networks they attach to.
...
There is no easy way of finding the IP ranges of the Kubernetes primary network. You could just check the IP addresses of individual cluster nodes, from the Node resource instances, and guess which is their subnet range, or ask an OpenShift Administrator.

// JFYI the web console lists "management address" for nodes, which is for BMC, not for the primary network

From the perspective of Kubernetes, its primary network does not need to be an isolated network. Kubernetes only requires IP connectivity between its cluster nodes over a wide range of TCP ports. There are additional latency considerations between Kubernetes control plane nodes, but they are beyond the scope of this course.
...
* OpenShift Route resources for public API entry points. All accesses from OpenStack clients outside the cluster, and to the Horizon dashboard, come through OpenShift routes and are mediated by the OpenShift Ingress controller.
...
* Kubernetes Service resources, as internal load balancers, for components of OpenStack services which require no access from outside an OpenShift cluster, and are accessed only by other Pods.
...
In addition to Services and Routes, Pods from Red Hat Services on OpenShift require connectivity to multiple isolated networks to start network connections to storage backends and SSH management of compute nodes, which are not possible with standard Kubernetes networking but are possible with Multus, including with Red Hat OpenShift.
...
Those two OVN instances cannot mix with each other: one set of OVN pods manages Kubernetes networking, and another set of OVN pods manages OpenStack networking. Each requires a dedicated network interface for tunneling traffic between cluster nodes.
...
As you consider your isolated network design for OpenStack clusters, remember to consider the OpenShift cluster nodes network and also its internal Kubernetes primary network, to prevent IP address conflicts. Such conflicts would affect OpenStack Pods and be very hard to track.

Also consider the isolated network design of your storage backends, which might require additional isolated networks connected to either OpenShift or OpenStack cluster nodes, or maybe to both. As we will see in the next section, OpenShift and OpenStack clusters do not require connectivity to the same storage backends, but OpenStack Pods do require some connectivity to the same storage backends as OpenStack compute nodes.
...
Connects OpenShift cluster nodes to each other and to the outside world. This is the network that tunnels all traffic between Kubernetes Pods, in all namespaces, thanks to the OVN pods from OpenShift.
...
A common simplification, especially for smaller clusters, is using the Kubernetes primary network as the OpenStack external network. This works because individual Kubernetes cluster nodes require connectivity to the outside world and that traffic is not usually blocked by a firewall.
