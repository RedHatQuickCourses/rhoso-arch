...
_Incorrect: While OpenShift enables the use of Ingress Services to provide well knwon virtual IP addresses to applications, they are not required and all Pods have network connectivity with outside their cluster._
...
_Incorrect: Kubernetes Services of type Cluster IP only provide connectivity with other Pods inside the same Kubernetes cluster and do not provide conenctivity with external web clients nor external video devices._
...
_Incorrect: OpenShift Routes cannot support non-HTTP protocols such as SSH. Anyway, Routes provides ingress traffic to Pods, while SSH connections are eggress connections from Pods to RHEL compute nodes._
