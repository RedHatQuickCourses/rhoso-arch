...
Later in this course we will present the Kubernetes concepts and API resources required to manage storage for OpenStack services, and which OpenStack internal services services do require Kubernetes storage.
...
To provide network connectivity capabilities beyond standard Kuberntes, OpenShift includes a cluster operator and to add-on operators:
