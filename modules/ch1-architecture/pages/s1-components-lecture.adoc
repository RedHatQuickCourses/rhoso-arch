...
For now, let's ignore the Data Plane services, which are required on compute nodes to act on behalf of OpenStack services. These data plane services run as podman containers on RHEL compute nodes and are started as systemd units. Let's focus on the control plane and how it runs on Red Hat OpenShift.
...
The purpose of an add-on operator is encoding and automating all operational knowledge necessary to manage an application of other IT infrastructure, hence the name "operator" for this piece of software.
...
OpenStack Administrators might either be OpenShift Administrators themselves or work close to a dedicated team of OpenShift Administrators. In any case, an OpenStack Administrator using Red Hat OpenStack Services on OpenShift must be a capable OpenShift Operator, because OpenStack Administrators manage a very specific set of application workloads running on OpenShift: the OpenStack control plane.
...
The following table compares the Kubernetes and OpenStack from the point of view of their respective operator personas. It servers as a quick overfiew for experienced OpenStack Operators which must start their journeys to become also OpenShift Operators.
...
| Unique resource name scoped for an entire Kubernetes cluster or to a single namespace. OpenShift adds projects as self-service namespaces.
