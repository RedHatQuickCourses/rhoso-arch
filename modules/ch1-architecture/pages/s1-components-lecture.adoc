...
Manages message queues, which OpenStack services use to either broadcast events or for asynchronous remote procedure calls (RPC) between internal components of a service, including both containers running on OpenShift and on RHEL compute nodes.

OVN::

Provides a software-defined networking layer that enables tenant networks and other features from Neutron without requiring special-purpose hardware.
...
* Application development services such as CI/CD pipelines.

It's very common for add-on operators to be designed to manage other software running on Kubernetes, for example, a database server management operator. A basic database add-on operator would manage just the deployment of a database server and its continued availability. A more sophisticated database add-on operator would also manage backups of database data, replication of such data from a read-write to multiple read-only replicas, and promotion of a read-only replica to a read-write one in case the old read-write replica fails.
...
The OpenStack operator also manages a number of child operators, among them the *External Data Plane Management Operator* which runs Ansible playbooks to manage OpenStack compute nodes. OpenStack Administrators usually do not interact directly with the child operators of the OpenStack operator.
...
== OpenStack Administrators and OpenShift
...
Kubernetes manages workloads as containers, while OpenStack manages workloads as Virtual Machines (VMs). Kubernetes orchestrates running multiple containers over a cluster of compute nodes and provides these containers with basic storage and networking resources. What Kubernetes does for containers seems, on the surface, very similar to what OpenStack does for VMs.
...
Do not try to understand all the terms of that table right now. Later in this course, we will introduce each of the Kubernetes concepts mentioned in it, as well as  all the features from OpenShift that are required by Red Hat OpenStack Services on OpenShift.
...
| Cinder VM images and Heat templates.
...
| Set of Kubernetes API resources which include basic computing, storage, and networking capabilities.
...
h| API style
| Imperative: API requests perform actions that change workloads directly but asynchronously.
| Declarative: API requests change the state of API resources and a resource controller later reconciles workloads to match the new state.
...
| Unique resource names are scoped to either an entire Kubernetes cluster or to a single namespace. OpenShift adds projects for self-service namespace management.
...
| Persistent Volume Claims (PVCs), storage classes.
